...
public class SellService {
	 ...
	 */
	public List<SellVO> getProducts(String query, int offset, int limit) {
	    // 조회 조건을 저장할 맵 생성
	    ...
	    return sellDAO.selectProducts(param);
	}
	 ...
	 */
	public List<SellVO> getProductsWithPhoto(String query, int offset, int limit) {
	    // 판매 상품 목록 조회
	    List<SellVO> list = getProducts(query, offset, limit);

	    // 각 상품에 대해 대표 사진 설정
	    for (SellVO vo : list) {
	        // 판매 상품 ID로 첫 번째 사진 조회
	        PhotoVO photo = sellDAO.selectFirstPhotoBySellId(vo.getSellId());
     ...
     */
    public SellVO getProductDetails(int sellId) {
        // 상품 상세 정보 가져오기
        return sellDAO.selectProductDetails(sellId);
    }
     ...
     */
    public List<PhotoVO> getProductPhotos(int sellId) {
        // 상품에 연결된 모든 사진 목록 가져오기
        return sellDAO.selectProductPhotos(sellId);
    }
	
    
    public AutoCompleteVO getAutoComplete(String cropType) {
        return sellDAO.autoText(cropType);
    }
